...
import {adminCorrectBasenGodziny} from "../modules/basen/basen_godziny_service";
...
/**
 * POST /api/basen/admin/godziny/correct
 * Admin wykonuje korektę godzin basenowych (plus lub minus).
 * Body: { userUid: string, amount: number, reason?: string }
 * amount > 0 → korekta plus, amount < 0 → korekta minus
 */
export async function handleBasenAdminCorrectGodziny(req: Request, res: Response, deps: Deps): Promise<void> {
  ...
  deps.corsHandler(req, res, async () => {
    ...
    try {
      ...
      if (amount === 0 || isNaN(amount)) {
        res.status(400).json({error: "Kwota korekty (amount) nie może być zerowa."});
        return;
      }
      ...
      const recordId = await adminCorrectBasenGodziny(deps.db, {
        uid: targetUid,
        amount,
        reason: reason || "Korekta godzin przez admina",
        performedBy: adminUid,
      });

      res.status(200).json({ok: true, recordId, correctionType: amount > 0 ? "plus" : "minus"});
    } catch (err) {
