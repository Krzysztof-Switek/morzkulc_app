// Ręczne uruchomienie events.syncFromSheet (migracja kierownikUid -> kierownikUids
// po wdrożeniu wielu-kierowników-per-impreza) — czeka na wynik.
// node --use-system-ca scripts/enqueueEventsSyncFromSheet.js [--dry]
...
(async () => {
  const id = `manual-events-sync-${Date.now()}`;
  const ref = db.collection("service_jobs").doc(id);
  await ref.set({
    taskId: "events.syncFromSheet",
    payload: { dry },
    status: "queued",
    attempts: 0,
